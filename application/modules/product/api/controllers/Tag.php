...
class Tag extends Api_Controller {
	...
	public function create_post() {
		$data = $this->post();
		$rules = $this->common_product_tag->rules;
		$rules[0]['rules'] .= '|is_unique[' . $this->common_product_tag->table . '.tag_title]';
		$rules[1]['rules'] .= '|is_unique[' . $this->common_product_tag->table . '.tag_url]';

		$this->form_validation->set_data($data);
		$this->form_validation->set_rules($rules);
		if ($this->form_validation->run($rules)) {
			$result = $this->common_product_tag->insert(set_data($data));
			if ($result) {
	...
	public function update_post() {
		...
		$tag = $this->common_product_tag->get_by_id($id);
		...
		if ($data['tag_url'] !== $tag['tag_url']) {
			$rules[0]['rules'] .= '|is_unique[' . $this->common_product_tag->table . '.tag_url]';
		}
		...
		if ($this->form_validation->run($rules)) {
			$result = $this->common_product_tag->update(set_data($data), $id);
			if ($result) {
				return $this->response([
					'status' => REST_Controller::HTTP_OK,
					'message' => 'Tag updated',
	...
	public function get_tags_by_keyword_get() {
		...
		$tags = $this->common_product_tag->get_tags_by_keyword($keyword);
		return $this->response([
			'status' => REST_Controller::HTTP_OK,
			'data' => $tags,
		], REST_Controller::HTTP_OK);
	...
	public function check_tag_post() {
		...
		if ($this->common_product_tag->get(['tag_title' => $tag_title])) {
			$this->response($result, REST_Controller::HTTP_OK);
		} else {
			$result = $this->common_product_tag->insert(['tag_title' => $tag_title, 'tag_url' => string_to_url($tag_title)]);
			if ($result) {
				return $this->response([
					'status' => REST_Controller::HTTP_OK,
					'message' => $result,
...
/* End of file Tag.php */
/* Location: ./application/modules/product/api/controllers/Tag.php */