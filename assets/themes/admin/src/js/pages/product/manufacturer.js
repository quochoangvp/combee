$(document).ready(function() {
    $('form').submit(function(event) {
        ...
        var method = 'POST';
        var manufacturer_id = parseInt($('input[name="manufacturer_id"]').val());
...
function go_to_manufacturer_page() {
    window.location.href = '/admin/product/manufacturer';
}

function confirm_delete_manufacturer(id) {
    var cf = confirm('Do you want to delete this manufacturer?');
    if (cf) {
        $.ajax({
            url: '/api/product/manufacturer',
            type: 'DELETE',
            dataType: 'json',
            data: { id: id },
        })
